const PREFIXES = ['eiken4', 'grade1', 'grade2', 'grade3'];
...
export const createTransfer = () => {
  ...
  if (typeof localStorage !== 'undefined') for (let index = 0; index < localStorage.length; index += 1) {
    const key = localStorage.key(index); if (key && PREFIXES.some(prefix => key.startsWith(prefix))) values[key] = localStorage.getItem(key) || '';
  }
...
export const importTransfer = (data: TransferPackage) => {
  ...
  for (const [key, raw] of Object.entries(data.values)) { if (!PREFIXES.some(prefix => key.startsWith(prefix))) continue; try { const existing = localStorage.getItem(key); localStorage.setItem(key, existing ? JSON.stringify(merge(JSON.parse(existing), JSON.parse(raw))) : raw); } catch { localStorage.setItem(key, raw); } count += 1; }
  return count;
};
